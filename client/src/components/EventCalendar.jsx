...
const EventCalendar = () => {
  ...
  return (
    <div className='flex flex-col items-center p-4 bg-gray-100 rounded-lg shadow-md w-full h-full'>
      ...
      <Calendar
        ...
        tileClassName={({ date }) => {
          if (eventDates.includes(date.toDateString())) {
            return 'bg-blue-500 text-white rounded-full'; // Tailwind classes for highlight
          }
          return null;
        }}
      />
      ...
      <div className='mt-4 w-full'>
        ...
        <ul className='list-none text-primary text-xl pl-5'>
          {events.filter(event => new Date(event.date).toDateString() === value.toDateString()).map(event => (
            <li key={event._id} className='text-primary text-bold'>
              {event.name}
            </li>
          ))}
        </ul>
